...
async def extract_text(
    ...
    """Try text layer first (free, instant), fall back to Azure DI. Returns (text, confidence, ocr_metadata).
    ...
    if mime_type in _IMAGE_MIMES or mime_type == "application/pdf":
        text, confidence, metadata = await azure_di_ocr(blob_url, file_path=file_path)
        if text:
